      ...
      defaultExtractor: (content) => content.match(/[\w-/:]+(?<!:)/g) || [],
        ...
        <div className="bg-senary p-1 my-2">
          <p className="text-md lh-normal font-medium text-base">
            when using any js framework like react js for example all you have
            to edit is the content array where you have to set the final build
            paths of files to be purged by the plugin , the idea is to build
            your final version with the framework normally then trigger the
            optimization build command , file when using reactJS will look like
            this:
          </p>
        </div>
  ...
  plugins: [
    ...
    "build:prod-css": "postcss ./src/style.css -d ./src/dis/purged.css "
        ...
        <p className="text-md lh-normal font-medium text-inner opacity-80">
          when using any js framework you have to combine the 2 build , the
          build of the framework and the optimisation build of the AkramCSS wich
          is (build:prod-css) , the final build will look like this for reactJS
          :
        </p>
